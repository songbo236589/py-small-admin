...
|------|------------|---------|------|
| 知乎 | ✅ 已支持 | ✅ 已支持 | 使用 Cookie 认证 + 完整反检测机制 |
| 小红书 | ✅ 已支持 | ✅ 已支持 | 支持原创声明、Markdown导入发布 |
| 今日头条 | ✅ 已支持 | ✅ 已支持 | 支持封面选择、作品声明、头条首发 |
...
| **页面刷新** | **启用** | **验证前刷新页面，确保 Cookie 完全生效** |
...
# 页面刷新配置（用于 Cookie 验证）
CONTENT_ENABLE_PAGE_REFRESH=true           # 是否在验证时刷新页面
CONTENT_PAGE_REFRESH_DELAY_MIN=2.0         # 刷新前最小延迟时间（秒）
CONTENT_PAGE_REFRESH_DELAY_MAX=4.0         # 刷新前最大延迟时间（秒）
CONTENT_PAGE_REFRESH_AFTER_DELAY_MIN=1.0   # 刷新后最小延迟时间（秒）
CONTENT_PAGE_REFRESH_AFTER_DELAY_MAX=2.0   # 刷新后最大延迟时间（秒）
```
...
4. **页面刷新**：验证前刷新页面，确保 Cookie 完全生效
5. **停留时间**：根据验证结果停留不同时长（成功 5-8 秒，失败 2-4 秒）

**页面刷新流程**：

```
访问验证页面 → 等待加载 → 模拟人类行为 → 【刷新页面】→ 检查登录状态
                    ↓                           ↓
              延迟 2-3 秒                 延迟 2-4 秒
                                           刷新前等待
                                           ↓
...
这些行为会自动执行，无需额外配置。如需禁用，将 `CONTENT_HUMAN_BEHAVIOR_ENABLED` 或 `CONTENT_ENABLE_PAGE_REFRESH` 设置为 `false`。
