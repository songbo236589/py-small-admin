...
import {
  ...
  syncList,
  syncRelation,
} from '@/services/quant/data/concept/api';
...
const Index: React.FC = () => {
  ...
  const columns: ProColumns<API.QuantConceptList>[] = [
    {
      title: '排名',
      dataIndex: 'sort',
      hideInSearch: true,
    ...
    },
    {
      ...
      render: (_, row) => [
        ...
        <Button
          ...
          onClick={() => {
            window.open(`/quant/data/concept_log?concept_id=${row.id}`, '_blank');
          }}
        >
          日志
        </Button>,
        <Button
          ...
          icon={<EyeOutlined />}
          onClick={() => {
            window.open(`/quant/data/stock?concept_id=${row.id}`, '_blank');
          }}
        >
          详情
        </Button>,
  ...
  return (
    <ProTableWrapper
      ...
      columnsState={{
        defaultValue: (() => {
          const stored = localStorage.getItem('quant_data_concept_table_columns');
          return stored ? JSON.parse(stored) : {};
        })(),
        onChange: (map: any) => {
          maps = map;
        },
        persistenceKey: 'quant_data_concept_table_columns',
