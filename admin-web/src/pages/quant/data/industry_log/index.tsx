...
import { simpleList } from '@/services/quant/data/industry/api';
import { destroyAll, getList } from '@/services/quant/data/industry_log/api';
...
const Index: React.FC = () => {
  ...
  const [chartData, setChartData] = useState<API.QuantIndustryLogList[]>([]);
  ...
  const fetchIndustryList = async () => {
    ...
    if (res.code === 200) {
      ...
      if (Object.keys(industryListData).length) await setIndustryList({ ...industryListData });
    }
  };
  ...
  // 转换数据为图表格式（长格式，支持多系列）
  const transformChartData = (data: API.QuantIndustryLogList[]) => {
  ...
  const columns: ProColumns<API.QuantIndustryLogList>[] = [
    {
      title: '排名',
      dataIndex: 'sort',
      hideInSearch: true,
    ...
    <>
      <ProTableWrapper
        ...
        request={async (params: any, sort) => {
          ...
          // 当搜索了行业名称时，保存数据用于图表展示
          if (params.industry_id && res.code === 200 && res.data.items) {
        ...
        columnsState={{
          defaultValue: (() => {
            const stored = localStorage.getItem('quant_data_industry_log_table_columns');
            return stored ? JSON.parse(stored) : {};
          })(),
          onChange: (map: any) => {
            maps = map;
          },
          persistenceKey: 'quant_data_industry_log_table_columns',
        ...
        <Card id="chart-card" style={{ marginBottom: 16, padding: '24px', position: 'relative' }}>
          ...
          <Tooltip title="下载图表" placement="bottom">
            <Button
              ...
              onClick={() => {
                chartRef.current.downloadImage('行业指标趋势图', 'image/png', 1);
              }}
              type="text"
