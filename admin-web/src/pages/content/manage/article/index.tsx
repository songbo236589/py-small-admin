...
import { destroy, destroyAll, getList } from '@/services/content/manage/article/api';
import { getTree as getCategoryTree } from '@/services/content/manage/category/api';
import { getList as getTagList } from '@/services/content/manage/tag/api';
import { getList as getTopicList } from '@/services/content/manage/topic/api';
...
const Index: React.FC = () => {
  ...
  const [topicOptions, setTopicOptions] = useState<{ label: string; value: number }[]>([]);
  ...
  const fetchTopicOptions = async () => {
    ...
    if (res.code === 200 && res.data?.items) {
      const options = res.data.items.map((item: API.ContentTopic) => ({
        label: item.title,
        value: item.id,
      }));
