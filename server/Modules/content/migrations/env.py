...
try:
    ...
    from Modules.content.models import *  # 导入模块中的所有模型
    ...
    print(f"警告: 无法导入模块 content 的模型: {e}")
...
def include_object(object, name, type_, reflected, compare_to):
    ...
    if type_ == "table" and name:
        ...
        # 检查表名是否属于当前模块
        expected_prefix = f"{table_prefix}content_"
    ...
    if type_ == "index" and name:
        ...
        # 直接检查索引名称中是否包含模块的表名
        expected_table_name = f"{table_prefix}content"
...
def run_migrations_offline() -> None:
    ...
    try:
        ...
        db_config = connections.get(default_connection, {})
        table_prefix = db_config.get("prefix", "fa_")
        version_table = f"{table_prefix}content_alembic_versions"
        print(f"使用版本表: {version_table}")
    except Exception as e:
        print(f"警告: 无法获取表前缀配置: {e}")
        version_table = f"fa_content_alembic_versions"
