...
class QuantConceptAddUpdateRequest(BaseModel):
    """概念添加和更新请求模型"""

    name: str = Field(..., description="概念名称")
    code: str | None = Field(None, description="概念代码")
    description: str | None = Field(None, description="概念描述")
    ...
    def validate_name(cls, v):
        ...
        if len(v) > 100:
            raise ValueError("概念名称长度不能超过100个字符")
        return v.strip()
    ...
    def validate_code(cls, v):
        ...
        v = v.strip()
        if len(v) > 50:
            raise ValueError("概念代码长度不能超过50个字符")
    ...
    def validate_description(cls, v):
        ...
        v = v.strip()
        if len(v) > 500:
            raise ValueError("概念描述长度不能超过500个字符")
        return v if v else None
